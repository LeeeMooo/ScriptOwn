const scriptName = "douyu"
const tokenUrl = "mcenter/myinfo"
const tokenKey = "douyu_token"
const cookieKey = "douyu_cookie"
const commonJS = MagicJS(scriptName)
let signNotify = ''

const daySignOptions = {
	...
	headers: {
		...
		"User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 14_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148, Douyu_IOS, Douyu_IOS",
		"Referer": "https://apiv2.douyucdn.cn/H5/Sign/info?client_sys=ios&ic=0"
	},
	body: ''
}

const yubaSignOptions = {
	url: "https://mapi-yuba.douyu.com/wb/v3/fastSign",
	headers: {
		"client": "ios",
		"User-Agent": "DYZB/6.340(iPhone;iOS 14.2;Scale/3.00)",
...
async function douyu() {
    ...
    let daySignRetry = commonJS.retry(daySign, 10, 500)
...
function daySign() {
	...
	const body = `client_sys=ios&did=${cookie.acf_did}&token=${commonJS.read(tokenKey, 'default')}`
	daySignOptions['body'] = body
	let log = ''
	let notify = ''
	return new Promise((resolve, reject) => {
		commonJS.post(daySignOptions, (err, res, data) => {
			if (err) {
				...
				switch (parseInt(error)) {
					case 0:
						notify = `${name} 连续签到${_data.sign_rd}天🎉`
						//signNotify += notify + "\r\n"
						break;
					case 6305:
						notify = `${name} 已签到⏰`
...
function yubaSign() {
	const name = "[鱼吧签到]"
	yubaSignOptions.headers['Token'] = commonJS.read(tokenKey, 'default')
	let log = ''
	let notify = ''
	return new Promise((resolve) => {
		commonJS.post(yubaSignOptions, (err, res, data) => {
			if (err) {
				notify = `${name} 签到异常,查看log`
				log = `${name} 异常: ${err}`
				commonJS.log(log)
				signNotify += notify + "\r\n"
				resolve(log)
				...
				const signData = JSON.parse(data)
				commonJS.log(data)
				let { status_code, message = "", data: _data } = signData
				if (parseInt(status_code) !== 200) {
					if (!message) {
						message = _data
					}
					notify = `${name} ${message}❗️`
					signNotify += notify + "\r\n"
				} else {
					notify = _data === 0 ? `${name} 已签到⏰` : `${name} 成功🎉`
					signNotify += notify + "\r\n"
				}
				resolve(notify)
			}
...
function getParam(body, outStr, inStr) {
	const obj = {}
	const splitBody = body.split(outStr)
	for (var i = 0, len = splitBody.length; i < len; i++) {
		const params = splitBody[i].split(inStr)
